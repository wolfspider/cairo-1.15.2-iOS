			       ...
			       cairo_scaled_glyph_t *scaled_glyph)
{
	...
	textMatrix = CGAffineTransformMake (font->base.scale.xx / 2.0,
										font->base.scale.yx / 2.0,
										font->base.scale.xy / 2.0,
										font->base.scale.yy / 2.0,
										0, 0);
								  ...
								  cairo_scaled_glyph_t *scaled_glyph)
{
	...
	if (surface->width != 0 && surface->height != 0) {
		...
		//CTFontRef ctFont;
		//CGPoint invertedPoint;
		
		//ctFont = CTFontCreateWithGraphicsFont(font_face->cgFont, 1.0, &textMatrix, NULL);
		
		//invertedPoint = CGPointMake(- glyphOrigin.x, - glyphOrigin.y);
		
		CGContextShowGlyphsAtPoint (cgContext, - glyphOrigin.x, - glyphOrigin.y, &glyph, 1);
		//CTFontDrawGlyphs(ctFont, &glyph, &invertedPoint, 1, cgContext);
